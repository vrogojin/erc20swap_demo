...
import './libraries/TestUniswapV2Library.sol';
...
contract TestUniswapV2Router01 is IUniswapV2Router01 {
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) private {
        for (uint i; i < path.length - 1; i++) {
            ...
            IUniswapV2Pair(TestUniswapV2Library.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, TestUniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]);
        ...
        returns (uint[] memory amounts)
    {
        ...
        assert(IWETH(WETH).transfer(TestUniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) public pure override returns (uint amountIn) {
        return TestUniswapV2Library.getAmountOut(amountOut, reserveIn, reserveOut);
    }
